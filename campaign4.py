...
def getKPIByCountry(df):
    ...
    df= df[['COUNTRY_NAME', 'CAMPAIGN','AD_TYPE','AD_NAME', 'CTR', 'VIEWS','25% VIEWED','50% VIEWED','75% VIEWED','VIDEO_COMPLETION_RATE']].sort_values(['COUNTRY_NAME'])
    return df
...
def getTableCountryPerf(df):
    ...
    AgGrid(df, gripOption, enable_enterprise_modules=True,fit_columns_on_grid_load=True,height=442,custom_css=custom_css,allow_unsafe_jscode=True, update_mode=GridUpdateMode.NO_UPDATE )
